...
import localWidgetRenderer from './local-widget-renderer.js';
...
class StencilStart {
    ...
    async run(cliOptions) {
        ...
        await this.startBrowserSync(cliOptions, browserSyncPort, resolvedWidgetsDir);
    }
     ...
     * @return {Promise<any>}
     */
    async startLocalServer(cliOptions, stencilConfig, widgetBuilderOpts = {}) {
        return Server.create({
            ...
            widgetsDir: widgetBuilderOpts.widgetsDir,
            apiHost: widgetBuilderOpts.apiHost,
            storeHash: widgetBuilderOpts.storeHash,
        });
    }

    async startBrowserSync(cliOptions, browserSyncPort, widgetsDir) {
        ...
        if (widgetsDir) {
            this._browserSync.watch(
                widgetsDir,
                { ignoreInitial: true },
                (event, filePath) => {
                    if (event === 'change') {
                        if (path.basename(filePath) === 'placements.mjs') {
                            localWidgetRenderer.invalidateAllCaches();
                            this._logger.log('placements.mjs changed — reloading all widgets'.cyan);
                        } else {
                            const widgetDir = path.dirname(filePath);
                            localWidgetRenderer.invalidateCache(widgetDir);
                            this._logger.log(`Widget file changed: ${path.basename(filePath)}`.cyan);
                        }
                        this._browserSync.reload();
                    }
                },
